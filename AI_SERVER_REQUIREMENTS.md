...
> **중요**: AI 서버는 백엔드와 **동일한 데이터베이스를 공유**하며 직접 DB에 접근합니다.

---

## 📌 AI 서버 개요

AI 서버는 다음과 같은 역할을 수행합니다:
1. **예측 모델 실행**: TFT 모델을 사용한 가격 예측
2. **데이터 수집**: 뉴스, 시장 지표 등 외부 데이터 수집
3. **LLM 분석**: 예측 설명 및 고영향 뉴스 분석
4. **데이터 저장**: 공유 DB에 직접 데이터 저장
5. **시뮬레이션 API**: 특정 조건 변경 시 예측 재계산

### 🔑 핵심 차이점
- ❌ 백엔드 API 호출 없음 (HTTP 요청 X)
- ✅ SQLAlchemy로 DB에 직접 접근
- ✅ 백엔드와 **동일한 스키마 공유** (`app/datatable.py`, `app/dataschemas.py`)
- ✅ 시뮬레이션만 FastAPI 엔드포인트로 제공
